...
const People = ({ chatScreenPersonId, userInfo }) => {
  ...
  return (
    <div
      className="w-full flex items-center gap-2 text-black px-4 py-2 my-2 rounded-xl cursor-pointer"
      style={{ backgroundColor: colors.primary }}
      id={`person-${chatScreenPersonId}`}
      onClick={() => {
      ...
      <div className="flex flex-col justify-evenly w-full">
        <div className="flex justify-between items-center w-[95%]">
          ...
          <span className="text-[0.65rem]">
            Last seen at {userInfo.lastseen}
          </span>
        </div>
...
const PeoplesComp = ({ searchText, userInfo }) => {
  const _peoples = [];
  console.log(searchText);
  for (const [key, value] of Object.entries(userInfo)) {
    if (searchText && value?.name.toLowerCase().includes(searchText)) {
      _peoples.push(
        <People chatScreenPersonId={key} userInfo={userInfo[key]} key={key} />
      );
    } else if (!searchText) {
      _peoples.push(
        <People chatScreenPersonId={key} userInfo={userInfo[key]} key={key} />
    ...
    <>
      <div className="pb-3 w-full flex flex-col items-center">
        ...
        <input
          ...
          className=" w-full max-w-[80%] py-2 px-4 rounded-full outline-none shadow-md text-gray-900"
          style={{ backgroundColor: colors.primary }}
        ></input>
      </div>
      <div className="overflow-scroll scroll-smooth no-scrollbar w-full flex items-center flex-col px-8">
        ...
        <div className="w-full">
          <PeoplesComp searchText={searchText} userInfo={userInfo} />
        </div>
      </div>
    </>
  );
