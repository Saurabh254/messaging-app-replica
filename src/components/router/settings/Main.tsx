...
const SettingsPage = ({ loggedInUser }) => {
  ...
  return (
    <div className="flex justify-between w-full h-full gap-8">
      <div
        className="flex flex-col items-start w-1/6 [&>*]:px-4 [&>*]:py-2 min-w-[250px] [&>*]:w-full text-center text-white"
        style={{ backgroundColor: colors.secondary }}
        onClick={(e) =>
          e.target.classList[0] !== settingState
            ? setSettingState(e.target?.classList[0])
            : null
        }
      >
